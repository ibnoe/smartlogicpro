...
@section('breadcrumb')
    ...
    <li><a href="{{ route('karyawan.penjualanfakturlengkaps.index') }}">Penjualan faktur lengkap</a></li>

@stop

@section('content')
    {{ Form::open(array('url' => route('karyawan.penjualanfakturlengkaps.store'), 'method' => 'post', 'class'=>'uk-form uk-form-horizontal ')) }}
	...
	<div class="uk-form-row {{ ($errors->has('npwp') ? 'has-error' : '') }}">
		...
		{{ $errors->first('npwp', 'Npwp harus kurang dari 25 karakter') }}
	</div>
